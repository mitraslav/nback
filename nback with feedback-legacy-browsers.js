...
async function experimentInit() {
  ...
  window.letters=["A", "B", "C", "D", "E", "F", "G", "H"];
  window.numbers=["1", "2", "3", "4", "5", "6", "7", "8"];
  
  window.n=2;
  window.number_trials=30;
  window.targetProbability=0.3;
  
  window.trialGroup=parsInt(expInfo["group"],10)||1;
  ...
  text_2 = new visual.TextStim({
    win: psychoJS.window,
    name: 'text_2',
    text: 'Подождите, пока данные загрузятся',
    font: 'Arial',
    units: undefined, 
    pos: [0, 0], draggable: false, height: 0.05,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
...
function trialRoutineBegin(snapshot) {
  return async function () {
    ...
    let target=Boolean(window.trialIndex]);
...
function trialRoutineEachFrame() {
  ...
  function trialRoutineEnd(snapshot) {
    return async function () {
      ...
      psychoJS.experiment.addData("key_pressed_json", JSON.stringfy(key_pressed));
    ...
    return async function () {
      ...
      fetch('https://pipe.jspsych.org/api/data', {
          ...
          headers: {
           ...
           body: JSON.stringify({
              experimentID: 'I1uQwmOPi64y', // * DATAPIPE EXP ID*
              filename: filename, 
              data: data,
           }),
